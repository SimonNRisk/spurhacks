  ...
  const handleRangeSelect = (range: { from: Date; to?: Date } | undefined) => {
    ...
    while (day <= end) {
      for (const unavailable of unavailableRanges) {
        ...
        const unEnd = new Date(
          new Date(unavailable.end).setDate(
            new Date(unavailable.end).getDate() + 1
          )
        );
    ...
    if (!conflict) {
      ...
      alert(
        "That range includes unavailable dates. Please choose a different range."
      );
    }
  };
                  ...
                  <Calendar
                    ...
                    disabled={(date) => {
                      ...
                      for (const range of unavailableRanges) {
                        ...
                        const end = new Date(
                          new Date(range.end).setDate(
                            new Date(range.end).getDate() + 1
                          )
                        );
