...
import { Calendar, Package, DollarSign, TrendingUp, Clock, CheckCircle, Eye, Star, User } from 'lucide-react';
...
//   },
  upcomingRentals: [
    ...
    { id: 3, item: 'Power Drill', renter: 'Tom Brown', date: '2025-07-02', duration: '2 days', price: 45, status: 'confirmed' }
  ],
  pastRentals: [
    { id: 4, item: 'Mountain Bike', owner: 'Alex Chen', date: '2025-06-15', duration: '2 days', price: 75, rating: 5 },
    { id: 5, item: 'Projector', owner: 'Lisa Park', date: '2025-06-10', duration: '1 day', price: 60, rating: 4 },
    { id: 6, item: 'Pressure Washer', owner: 'David Lee', date: '2025-06-05', duration: '3 days', price: 120, rating: 5 },
    { id: 7, item: 'Wedding Dress', owner: 'Emma Smith', date: '2025-05-28', duration: '1 day', price: 200, rating: 5 }
  ],
  listedItems: [
    { id: 1, name: 'Professional Camera Kit', category: 'Electronics', price: 60, views: 245, bookings: 8, status: 'active' },
    { id: 2, name: 'Camping Tent Set', category: 'Outdoor', price: 25, views: 189, bookings: 12, status: 'active' },
    { id: 3, name: 'Power Drill', category: 'Tools', price: 15, views: 156, bookings: 6, status: 'active' },
    { id: 4, name: 'Party Speakers', category: 'Electronics', price: 40, views: 298, bookings: 15, status: 'active' },
    { id: 5, name: 'Yoga Mats (5x)', category: 'Fitness', price: 20, views: 98, bookings: 4, status: 'inactive' }
  ],
  pendingRequests: [
    { id: 1, image: 'https://images.unsplash.com/photo-1519181245277-cffeb31da2b4?w=100&h=100&fit=crop', title: 'Professional Camera Kit', user: 'Emily Davis', startDate: '2025-06-20', endDate: '2025-06-23', message: 'Hi, I would love to use this for my project.' },
    { id: 2, image: 'https://images.unsplash.com/photo-1504280390367-361c6d9f38f4?w=100&h=100&fit=crop', title: 'Camping Tent Set', user: 'Jason Lee', startDate: '2025-06-30', endDate: '2025-07-05', message: 'Planning a weekend camping trip.' }
  ]
};
...
const StatCard = ({ title, value, icon: Icon, color, delay = 0, prefix = '', suffix = '' }) => {
  ...
  return (
    <Card className={`relative transition-all duration-700 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'} hover:scale-105 hover:shadow-lg cursor-pointer border-l-4`} 
      ...
      <CardContent className="p-6">
        ...
        <div className="pr-12">
          ...
          <p className="text-3xl font-bold" style={{ color }}>
            {isVisible ? <AnimatedCounter value={value} prefix={prefix} suffix={suffix} /> : '0'}
          </p>
        </div>
      </CardContent>
    </Card>
...
const RentalCard = ({ rental, type = 'upcoming' }) => {
  ...
  return (
    <div className="p-1"> {/* Container with padding for ring space */}
      <Card 
        className={`transition-all duration-300 transform hover:scale-102 hover:shadow-md cursor-pointer ${isHovered ? 'ring-2 ring-blue-500' : ''}`}
...
const ItemCard = ({ item }) => {
  ...
  return (
    <div className="p-1"> {/* Container with padding for ring space */}
      <Card 
        className={`transition-all duration-300 transform hover:scale-102 hover:shadow-md cursor-pointer ${isHovered ? 'ring-2 ring-purple-500' : ''}`}
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
      >
        <CardContent className="p-4">
...
const PendingRequestCard = ({ request }) => {
  ...
  return (
    <div className="p-1 w-full">
      <Card className={`transition-all duration-300 transform hover:scale-102 hover:shadow-md cursor-pointer ${isHovered ? 'ring-2 ring-blue-500' : ''}`} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
        <CardContent className="p-4 flex items-center gap-4">
          ...
          <div className="flex flex-col space-y-2">
            ...
            <button className="px-3 py-1 bg-red-500 text-white rounded-md hover:bg-red-600 text-sm">Reject</button>
          </div>
        </CardContent>
      </Card>
    </div>
